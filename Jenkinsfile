    ...
    stage('Docker build') {
      container('docker') {
        withDockerRegistry(credentialsId: 'dockerCredentials', url: "https://${DOCKER_REGISTRY_DOWNLOAD_URL}") {  //TODO
          sh """
            docker build --build-arg BASE_IMAGE=ossim-runtime-minimal-alpine:dev --network=host -t "${DOCKER_REGISTRY_PUBLIC_UPLOAD_URL}"/omar-stager-app:${BRANCH_NAME} ./docker
          """
        }
      }
    }
